...
/**
 * A SearchStaticFilter on Field[disabled]. If set it filters on Field[disabled]=false. 
 * Otherwise, the filter is ignored.
 * @author kprins
 */
public class DisabledStaticFilter<TBean extends Serializable> 
  ...
  /**
   * {@inheritDoc} <p>OVERRIDE: Return a EntityFilter on the Field[disabled] if TBean
  ...
  protected EntityFilter<TBean> onGetFilter() {
    ...
    try {
      if (this.getCurrentSearchValue()) {
        bResult = new EntityBoolFilter<>("disabled",false,EntityFilterEnums.EQUAL);
      }
